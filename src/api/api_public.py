...
async def get_properties(
    ...
    try:
        ...
        # Enrich with entity_name by mapping parent_entity_id -> entities.entity_name
        enriched_properties = []
        ...
        response_data = {
            "properties": enriched_properties if enriched_properties else (result.data if result else []),
...
# ========================= Create/Insert Endpoints =========================

class CreateEntityRequest(BaseModel):
    entity_name: str = Field(..., description="Entity name")
    entity_type: Optional[str] = Field("parent entity", description="parent entity | sub-entity | single-property entity")
    state: Optional[str] = None
    jurisdiction: Optional[str] = None
    parent_entity_id: Optional[str] = Field(None, description="Required when entity_type is sub-entity")
    entity_id: Optional[str] = Field(None, description="Optional business ID; autogenerated if omitted")
    close_date: Optional[str] = None
    amount_due: Optional[float] = None
    previous_year_taxes: Optional[float] = None
    extraction_steps: Optional[str] = None
    account_number: Optional[str] = None
    property_address: Optional[str] = None
    tax_bill_link: Optional[str] = None

    @field_validator("entity_type")
    @classmethod
    def normalize_entity_type(cls, v: Optional[str]) -> Optional[str]:
...
@app.post("/api/v1/entities", response_model=CreateEntityResponse, tags=["Entities"])
@rate_limit("30/minute")
async def create_entity(request: Request, payload: CreateEntityRequest):
    """Create a new entity record."""
    try:
        data = payload.model_dump(exclude_none=True)

        # Basic validation for sub-entity linkage
        if data.get("entity_type") == "sub-entity" and not data.get("parent_entity_id"):
            raise HTTPException(status_code=400, detail="parent_entity_id is required for sub-entity")

        # Autogenerate a business entity_id if not provided
        if "entity_id" not in data:
            import uuid
            data["entity_id"] = f"ent_{uuid.uuid4().hex[:8]}"

        # Insert via table API
        result = await monitored_query(
            "create_entity",
            lambda: supabase.table("entities").insert(data).execute(),
            timeout=3.0
        )
...
class CreatePropertyRequest(BaseModel):
    ...
    jurisdiction: Optional[str] = None
    state: Optional[str] = None
    property_type: Optional[str] = Field("property", description="Logical type for the record")
    account_number: Optional[str] = None
    tax_bill_link: Optional[str] = None
    ...
    extraction_steps: Optional[str] = None
    close_date: Optional[str] = None
    parent_entity_id: Optional[str] = Field(None, description="Link to entities.entity_id")
...
@app.post("/api/v1/properties", response_model=CreatePropertyResponse, tags=["Properties"])
@rate_limit("30/minute")
async def create_property(request: Request, payload: CreatePropertyRequest):
    """Create a new property record."""
    try:
        data = payload.model_dump(exclude_none=True)

        # Insert via table API; rely on DB constraints for uniqueness
        result = await monitored_query(
            "create_property",
            lambda: supabase.table("properties").insert(data).execute(),
            timeout=3.0
        )
